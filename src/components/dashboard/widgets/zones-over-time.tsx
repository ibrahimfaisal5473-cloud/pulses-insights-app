...
import {
  LOW_SAMPLE_CHECKS,
  type ZonesHappinessTimeseriesResponse,
  type ZonesTimeseriesResponse,
} from "@/types";
...
function VisitsOverTime() {
  ...
  return (
    <WidgetCard
      ...
      query={query}
    >
      {(data) => <VisitsBody data={data} />}
    </WidgetCard>
  );
}

/**
 * Extracted from the render prop so the flatten/totals pass can be memoised
 * on the query data — it walks every zone across every day in the range, and
 * a hook can't be called inside the callback.
 */
function VisitsBody({ data }: { data: ZonesTimeseriesResponse }) {
  const { series, rows, totals } = useMemo(() => {
...
function HappinessOverTime() {
  ...
  return (
    <WidgetCard
      ...
      query={query}
    >
      {(data) => <HappinessBody data={data} />}
    </WidgetCard>
  );
}

/**
 * The heaviest derivation on this page — one cell, with its own colour and
 * title string, per zone per day in the range. Memoised on the query data so
 * it runs once per fetch rather than on every render.
 */
function HappinessBody({ data }: { data: ZonesHappinessTimeseriesResponse }) {
  const { columns, matrixRows, cells } = useMemo(() => {
    ...
    return {
      columns,
      matrixRows: rows.map((z) => ({ id: z.id, label: z.name })),
      cells,
    };
  }, [data.zones, data.timeseries]);
